...
def getpdb(pdb_entry, out_path):
    ...
    urllib.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/structure/%s.pdb" %
                       (pdb_entry, pdb_entry), out_file)
...
def getchothpdb(pdb_entry, out_path):
    ...
    """
    out_file = os.path.join(out_path, "%s.pdb" % pdb_entry)
    urllib.urlretrieve("http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/structure/chothia/%s.pdb" %
                       (pdb_entry, pdb_entry), out_file)
    if os.path.isfile(out_file):
        Retrieved = open(out_file).read()
        if not Retrieved.count("ATOM"):
...
def getsequence(entry, fab_list, out_path):
    ...
    urllib.urlretrieve(
        "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/sequences/%s_raw.fa" % (entry, entry), out_file)
    if os.path.isfile(out_file):
        Retrieved = open(out_file).read()
    ...
    for fab in fab_list:
        ...
        if Hchain != "NA":
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/sequences/%s_%s_VH.fa" % (entry, entry, Hchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
                if not Retrieved.count(">%s" % entry):
                    print "Failed to retrieve sequence file from SAbDab."
        ...
        if Lchain != "NA":
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/sequences/%s_%s_VL.fa" % (entry, entry, Lchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
                if not Retrieved.count(">%s" % entry):
                    print "Failed to retrieve sequence file from SAbDab."
...
def getannotation(entry, fab_list, out_path):
    ...
    for fab in fab_list:
        ...
        if Hchain != "NA":
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/annotation/%s_%s_VH.ann" % (entry, entry, Hchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
        ...
        if Lchain != "NA":
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/annotation/%s_%s_VL.ann" % (entry, entry, Lchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
...
def getabangle(entry, fab_list, out_path):
    ...
    for fab in fab_list:
        if "NA" in fab:
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/abangle/%s.abangle" % (entry, entry), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
...
def getimgt(entry, fab_list, out_path):
    ...
    for fab in fab_list:
        ...
        if Hchain != "NA":
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/imgt/%s_%s_H.imgt" % (entry, entry, Hchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
        ...
        if Lchain != "NA":
            ...
            urllib.urlretrieve(
                "http://opig.stats.ox.ac.uk/webapps/abdb/web_front/data/entries/%s/imgt/%s_%s_L.imgt" % (entry, entry, Lchain), out_file)
            if os.path.isfile(out_file):
                Retrieved = open(out_file).read()
                if not Retrieved.count("gene_type"):
                    print "Failed to retrieve imgt file from SAbDab."
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="sabdab_downloader", description=description,
                                     epilog=epilogue, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--summary_file', '-s', type=str,
                        help="A tab-separated csv downloaded from SAbDab - opig.stats.ox.ac.uk/webapps/sabdab.", dest="summary_file")
    parser.add_argument('--output_path', '-o', type=str,
                        help="The path to the output directory.", dest="output_path")
    parser.add_argument('--original_pdb', action="store_true",
                        help="Download the pdb structure(s).", dest="original_pdb")
    parser.add_argument('--chothia_pdb', action="store_true",
                        help="Download the chothia re-numbered pdb structure(s).", dest="chothia_pdb")
    parser.add_argument('--sequences', action="store_true",
                        help="Download the sequence information.", dest="sequence")
    parser.add_argument('--annotation', action="store_true",
                        help="Download the chothia numbered sequence information.", dest="annotation")
    parser.add_argument('--abangle', action="store_true",
                        help="Download the abangle angles.", dest="abangle")
    parser.add_argument('--imgt', action="store_true",
                        help="Download the imgt information for the structure.", dest="imgt")
