...
def get_list_dict_lengths(list_dict):
    list_dict_lengths = {}
    for key in list_dict.keys():
        list_dict_lengths[key] = len(list_dict[key])
    return list_dict_lengths
...
short2long = {}
for rsd in list(long2short.keys()):
    short2long[long2short[rsd]] = rsd

long2short_MSE = dict(**long2short, MSE='M')


def aa_long2short(aa_code):
    if aa_code in long2short:
        return long2short[aa_code]
    return 'X'
            ...
            if C in coords1 and N in coords2:
                ...
                if dis > maxdis:
                    if verbose or not allow_chainbreaks:
                        print('ERROR chainbreak:', chain,
                              res1, res2, dis, pdbfile)
...
def update_derived_data(pose):
    ...
    # not interspersed: each chain comes once
    ...
    assert len(chainbounds) == len(chains) + \
        1  # 0 at the beginning, N at the end
    if resids:
